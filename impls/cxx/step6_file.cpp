...
int main(int argc, char* argv[])
{
    ...
    repl_env.set("eval", new MalFunction([&repl_env](span<MalType*> arguments)
        {
            if (arguments.size() == 0)
                throw std::runtime_error("wrong argument count - expected 1");

            return EVAL(arguments[0], &repl_env);
        }));

    rep("(def! not (fn* (a) (if a false true)))", &repl_env);
    rep("(def! load-file (fn* (f) (eval (read-string (str \"(do \" (slurp f) \"\nnil)\")))))", &repl_env);

    MalList* args = new MalList();
    for (int i = 2; i < argc; ++i)
    {
        args->append(new MalString(argv[i]));
    }
    ...
    {
        std::stringstream ss;
        ss << "(load-file " << std::quoted(argv[1]) << ")";
        rep(ss.str(), &repl_env);
